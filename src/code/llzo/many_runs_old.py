...
#124124122222gs22sess2ssh
da_params = {'specie': 'Li', 'time_step': timestep, 'step_skip': step_skip, 'min_dt': 1}
    ...
    for i, slice in enumerate(range(0, 2000, 500)):
        ...
        for t, j in enumerate(p.disp_3d):
            if m == 0:
                ...
                msd_true[m, i, t] = np.sum(j[::8]**2, axis=-1).mean()
            else:
                n_disp_3d.append(j[m::8])
                msd_true[m, i, t] = np.sum(j[m::8]**2, axis=-1).mean()
