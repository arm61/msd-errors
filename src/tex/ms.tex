...
These variances are also typically unequal---the data are heteroscedastic \cite{smith_random_1996,he_statistical_2018}. 
...
\begin{figure}[htb]
    \centering
    \includegraphics[width=\columnwidth]{figures/stateff.pdf}
    \caption{
...
\begin{figure}
    ...
    \caption{
        ...
        Each simulation has been analysed as in Fig.~\ref{fig:msd}(a) and (b) to yield a single corresponding point estimate $\Dest$.
        The grey line shows the distribution of point estimates, $\prob{\Dest_\mathrm{num}}$, obtained using Bayesian regression with a mean vector and numerical covariance matrix derived from the complete dataset of all \num{4096} simulations.
        The pink horizontal bar shows an interval of one standard deviation in $\prob{\Dest}$.
...
\section{Numerical implementation in KINISI}
\label{sec:implementation}
%
We have implemented the approximate Bayesian estimation method described in the main text in the open-source Python package \textsc{kinisi}~\cite{mccluskey_kinisi_2022}. 
This section provides numerical details regarding the implementation of this method.
...
To estimate the variance of the observed MSD, we rescale the variance of observed squared displacements by the number of non-overlapping time intervals in the simulation, $\nind{i} = N_{\mathrm{atoms}} \times N_{t} / i$, as presented in Eqn.~\ref{equ:varestMSD}.  
...
These points can then be plotted as a histogram (as in Fig~\ref{fig:random_walk}), and summary statistics $\Dest$ and $\varest{\Dest}$ can be derived.
