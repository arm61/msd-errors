...
The distribution of estimated variances is biased relative to the true variance and skewed, due to numerical differences between the true covariance matrix $\mathbf{\Sigma}$ and the model covariance matrix $\mathbf{\Sigma^\prime}$.
...
\begin{enumerate}
    ...
    \item Analysis of the relative efficiency of the approximate Bayesian regression method, presented here, compared to OLS, WLS, and GLS, and the efficiency scaling of each method with number of mobile particles or total simulation time..
...
We can obtain improved statistics by averaging over statistically equivalent observed squared displacements (see Eqn.~2 in the main text), which can be acheived by averaging over mobile particles or by averaging over time origins.
...
\end{equation}
%.
Because each step is equally likely to move a partice along each of the $d$ dimensions, the term $n$ in Eqns.~\ref{equ:msd_SI} \&~\ref{equ:der_var} is replaced here with $n/d$.
...
\section{Scaling of $\var{\Dest}$ with simulation size and choice of estimator}
\label{sec:scaling}
%
Fig.~\ref{fig:stat_eff} presents an analysis of how $\var{\Dest}$ varies as a function of changing the number of mobile particles (Fig.~\ref{fig:stat_eff}a)and the total simulation time (number of steps) (Fig.~\ref{fig:stat_eff}b).
We compare the performance of four methods for estimating $\D$ from the observed MSD data: OLS, WLS, the approximate Bayesian regression method described here, and GLS.
Each data point is calculated as the variance over point-estimates of $\D$ from \num{512} individual 3D-lattice random walk simulations, for each combination of $N_\mathrm{atoms}$ and $\Delta t_\mathrm{max}$.
For WLS and GLS we compute the variances and covariance matrix, respectively, using the complete set of \num{512} simulations.
For the GLS dataset, this gives an optimally efficient estimator for linear regression of observed MSD data, corresponding to the Cram\'er--Rao lower bound of $\var{\Dest}$.

The approximate Bayesian regression method described in the main manuscript performs similarly to GLS with a converged exact covariance matrix, and gives significantly smaller uncertainty in $\Dest$ than OLS or WLS at all simulation sizes and lengths considered.
In addition, our method gives better scaling than OLS or WLS as the total simulation time is increased.

\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{figures/stat_eff.pdf}
    \caption{
        Statistical efficiency, $\var{\Dest}$, of OLS (pink), WLS (orange), GLS (blue), and our approximate Bayesian regression method (green), as a function of varying;
        the decrease in $\sigma^2[\hat{\mu}(\Dest_\alpha)]$ as a function of (a) number of atoms, and (b) total simulation time.
        The solid lines show a fitted power law relationship for each method.
        % Raw simulation data~\cite{mccluskey_zenodo_2022} and scripts~\cite{mccluskey_github_2022} to generate this figure are available under CC BY 4.0/MIT licences. 
        }
    \label{fig:stat_eff}
    \script{stat_eff.py}
...
As in the prior study of Burbano \emph{et al.}~\cite{burbano_sparse_2016}, simulations were performed using $2 \times 2 \times 2$ supercells with \SI{1536}{atoms} following the same simulation protocal as in Ref.~\citenum{burbano_sparse_2016}.
